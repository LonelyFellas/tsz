...
// 词条编辑整页表单的集成冒烟:mock 掉统一 data source,验证
...
vi.mock("./dataSource", () => ({
  adminWordsDataSource: dataSourceMocks
}));

const mockGet = dataSourceMocks.get;
const mockSave = dataSourceMocks.saveContent;
...
describe("WordEditor", () => {
  ...
  it("legacy 编辑路由遇到 V2 草稿时重定向向导且不挂载旧表单", async () => {
    ...
    expect(await screen.findByTestId("location")).toHaveTextContent(
      "/words/w-1/wizard/forms"
    );
    expect(screen.queryByText("基本词性")).not.toBeInTheDocument();
    expect(mockSave).not.toHaveBeenCalled();
  });

  it("legacy 编辑路由遇到已发布 V2 时重定向只读预览", async () => {
    mockGet.mockResolvedValue({
      word: {
        schema_version: 2,
        id: "w-1",
