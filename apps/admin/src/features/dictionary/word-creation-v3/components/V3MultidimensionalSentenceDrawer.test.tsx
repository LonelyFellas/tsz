...
function existingSentence(): WordSentenceWritableV3 {
  return {
    id: "sentence-existing-helper",
    level: "B1",
    en_text: {
      mode: "unified" as const,
      common: {
        id: "sentence-en-helper",
    ...
    zh_text_id: "sentence-zh-helper",
    zh_text: { version: 2 as const, text: "找到中心。", annotations: [] },
    zh_translations: [
      {
        id: "sentence-zh-helper",
        band: "b1_b2" as const,
        content: {
          version: 2 as const,
          text: "找到中心。",
...
describe("V3MultidimensionalSentenceDrawer", () => {
  ...
  it("自动发现返回 opaque next_cursor 后，加载更多改用相同片段的 selected cursor", async () => {
    resolveSentenceTargets
      .mockResolvedValueOnce({
        schema_version: 3,
        sentence_hash: "automatic-hash",
        ...
        range_results: [
          {
            ...
            segments_fingerprint: "central-location-range",
            normalized_surface: "central location",
            published_total: 2,
            published_matches: [],
            draft_matches: [],
            next_cursor: "opaque-discovery-cursor"
          }
        ]
      })
      .mockResolvedValueOnce({
    ...
    await waitFor(() =>
      expect(resolveSentenceTargets).toHaveBeenCalledTimes(2)
    );
    expect(resolveSentenceTargets.mock.calls[0]?.[0]).toMatchObject({
      mode: "all_published_targets"
    });
    expect(resolveSentenceTargets.mock.calls[1]?.[0]).toMatchObject({
      mode: "selected_segments",
      selected_segments: [{ start: 2, end: 18, surface: "central location" }],
      cursor: "opaque-discovery-cursor"
    });
  });
  ...
  it("一键发现把唯一词义自动加入，并按最长短语覆盖内部单词", async () => {
              ...
              results: [
                {
                  ...
                  matches: [],
                  senses: [
                    { sense_id: "target-sense", gloss: "墙的中心位置" },
                    { sense_id: "target-sense-empty", gloss: "" }
                  ]
                }
              ],
              total: 1,
    ...
    resolveSentenceTargets.mockResolvedValue({
      ...
      range_results: [
        {
          source_segments: [{ start: 0, end: 2, surface: "It" }],
          segments_fingerprint: "published-it",
          normalized_surface: "it",
          published_total: 1,
          published_matches: [
            {
              entry_id: "it-entry",
              publication_id: "it-publication",
              pos_id: "it-pos",
              base_form_id: "it-base",
              headword: "it",
              pos: "pronoun",
              matched_form_id: "it-form",
              matched_variant_id: "it-variant",
        ...
        {
          source_segments: [{ start: 3, end: 14, surface: "is centered" }],
          segments_fingerprint: "published-is-centered",
          normalized_surface: "is centered",
          published_total: 1,
          published_matches: [
            {
              entry_id: "is-centered-entry",
              publication_id: "is-centered-publication",
              pos_id: "is-centered-pos",
              base_form_id: "is-centered-base",
              headword: "is centered",
              pos: "phrase",
              matched_form_id: "is-centered-form",
              matched_variant_id: "is-centered-variant",
              matched_dialect: "common",
              matched_form_type: "base",
              component_usages: [],
              matches: [
        ...
        {
          source_segments: [{ start: 6, end: 14, surface: "centered" }],
          segments_fingerprint: "published-centered",
          normalized_surface: "centered",
          published_total: 1,
          published_matches: [
            {
              entry_id: "centered-entry",
              publication_id: "centered-publication",
              pos_id: "centered-pos",
              base_form_id: "centered-base",
              headword: "centered",
              pos: "phrase",
              matched_form_id: "centered-form",
              matched_variant_id: "centered-variant",
              matched_dialect: "common",
              matched_form_type: "base",
              component_usages: [],
              matches: [
    ...
    expect(
      screen
        .getAllByText("已关联")
        .map((node) => node.closest(".v3-sentence-drawer-association"))
        .find((node) => node?.textContent?.includes("center of the wall"))
    ).toBeTruthy();
    ...
    expect(onSave).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        associations: expect.arrayContaining([
          expect.objectContaining({
            target_word_id: "it-entry",
            target_sense_id: "it-sense",
            target_publication_id: "it-publication",
            target_form_variant_id: "it-variant"
          }),
          expect.objectContaining({
            target_word_id: "is-centered-entry",
            target_sense_id: "is-centered-sense",
            target_publication_id: "is-centered-publication",
            target_form_variant_id: "is-centered-variant"
          }),
          expect.objectContaining({
  ...
  it("发现能力关闭时仍完整展示 exact 关联成分并可本地删除", () => {
    render(
      <AntApp>
        <V3MultidimensionalSentenceDrawer
          initialAssociations={[
              ...
              target_component_usages: [
                {
                  ...
                  id: "component-center",
                  literal: "center",
                  target_word_id: "component-word",
                  target_publication_id: "component-publication",
                  target_pos_id: "component-pos",
                  target_base_form_id: "component-base",
                  target_sense_id: "component-sense",
                  target_form_id: "component-form",
                  target_variant_id: "component-variant",
          ...
          initialSentence={existingSentence()}
          onClose={vi.fn()}
          onSave={vi.fn()}
          open
          senseId="sense-1"
    ...
    expect(screen.getByText("句中词条发现能力尚未开启")).toBeVisible();
    expect(screen.getByText(/center · 暂无释义 · 通用 · 原形/u)).toBeVisible();
    fireEvent.click(screen.getByLabelText("删除关联 association-exact"));
    expect(
      screen.getByText("至少添加一条已关联词义或待关联词条")
    ).toBeVisible();
    expect(
      screen.getByRole("button", { name: "保存例句与关联" })
    ).toBeDisabled();
  });

  it("歧义候选由管理员选择具体词义后写入 exact identity", async () => {
    resolveSentenceTargets.mockResolvedValue({
      schema_version: 3,
      sentence_hash: "hash",
      ...
      range_results: [
        {
          source_segments: [{ start: 2, end: 10, surface: "location" }],
          segments_fingerprint: "location-range",
          normalized_surface: "location",
          published_total: 1,
          published_matches: [
            {
              ...
              headword: "location",
              pos: "noun",
              matched_form_id: "location-form",
              matched_variant_id: "location-variant",
              matched_dialect: "common",
              matched_form_type: "base",
              component_usages: [],
              matches: [
    ...
    fireEvent.change(screen.getByLabelText("中阶中文译文"), {
      target: { value: "一个地点出现了。" }
    });
    fireEvent.click(screen.getByText("一键发现"));
    fireEvent.click(await screen.findByText("查看 2 个词义"));
    fireEvent.click(screen.getByLabelText("关联词义：外景地"));
    fireEvent.click(screen.getByText("保存例句与关联"));

    expect(onSave.mock.calls[0]![0].associations).toEqual([
      expect.objectContaining({
        target_word_id: "location-entry",
        target_sense_id: "location-sense-2",
        target_publication_id: "location-publication",
        target_form_variant_id: "location-variant"
      })
    ]);
  });

  it("移除上下文词义只改抽屉内 links", async () => {
    const sentence = existingSentence();
    sentence.links.push({
      word_id: "context-word",
      sense_id: "context-sense",
      role: "context"
    });
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(
      <AntApp>
        <V3MultidimensionalSentenceDrawer
          initialAssociations={[
            {
              id: "association-pending-context",
              association_schema_version: 3,
              origin: "manual",
              source_dialect: "common",
              source_segments: [{ start: 9, end: 15, surface: "center" }],
    ...
    expect(screen.getByText("context-word · context-sense")).toBeVisible();
    fireEvent.click(screen.getByLabelText("删除词义归属 2"));
    fireEvent.click(screen.getByRole("button", { name: "保存例句与关联" }));
    expect(onSave.mock.calls[0]![0].sentence.links).toEqual([
      { word_id: "word-1", sense_id: "sense-1", role: "head" }
    ]);
  });

  it("旧单条译文按 CEFR 回填，v1 编辑与别名回退不丢格式", async () => {
    const sentence = existingSentence();
    sentence.level = "B1";
    sentence.zh_text_id = "translation-a1";
    sentence.zh_text = {
      version: 1,
      text: "旧译文",
      spans: [],
    ...
    fireEvent.change(screen.getByLabelText("高阶中文译文"), {
      target: { value: "更新后的高阶译文" }
    });
    fireEvent.click(screen.getByRole("button", { name: "保存例句与关联" }));
    expect(onSave.mock.calls[0]![0].sentence).toMatchObject({
      zh_text_id: "translation-a1",
      zh_text: {
        version: 1,
        text: "更新后的高阶译文",
        spans: [],
  ...
  it("无分层数组的 C1 旧译文回填为初阶，非 Error 保存失败使用兜底文案", async () => {
    const sentence = existingSentence();
    sentence.level = "C1";
    delete (sentence as Partial<WordSentenceWritableV3>).zh_translations;
    const onSave = vi.fn().mockRejectedValue("failed without Error");
    render(
      <AntApp>
        <V3MultidimensionalSentenceDrawer
          initialAssociations={[
            {
              id: "association-old-translation",
              association_schema_version: 3,
              origin: "manual",
              source_dialect: "common",
              source_segments: [{ start: 9, end: 15, surface: "center" }],
    ...
    expect(screen.getByLabelText("初阶中文译文")).toHaveValue("找到中心。");
    fireEvent.click(screen.getByRole("button", { name: "保存例句与关联" }));
    expect(await screen.findByText("例句关联保存失败")).toBeVisible();
  });

  it("手动不连续草稿可查看并转待关联，重复加入会提示重叠", async () => {
    ...
    resolveSentenceTargets.mockResolvedValue({
      ...
      range_results: [
        {
          source_segments: [
            { start: 5, end: 9, surface: "turn" },
            { start: 20, end: 23, surface: "off" }
          ],
          segments_fingerprint: "turn-off-range",
          normalized_surface: "turn off",
          published_total: 0,
          published_matches: [],
          draft_matches: [
            {
              entry_id: "draft-turn-off",
              entry_revision: 3,
              headword: "turn off",
              target_state: "draft",
    ...
    fireEvent.change(screen.getByLabelText("英文例句"), {
      target: { value: "They turn the light off." }
    });
    fireEvent.click(screen.getByLabelText("手动选择"));
    fireEvent.click(screen.getByLabelText("选择第 2 个词 turn"));
    fireEvent.click(screen.getByLabelText("选择第 5 个词 off"));
    fireEvent.click(screen.getByText("查询所选单词或短语"));
    fireEvent.click(await screen.findByText("查看草稿"));
    expect(open).toHaveBeenCalledWith(
      "/words/draft-turn-off/v3/wizard/meanings?mode=edit",
      "_blank",
      "noopener,noreferrer"
    );
    const convert = screen.getByText("转为待关联词条");
    ...
    fireEvent.click(convert);
    expect(
      screen.getByText("所选位置与本次关联中的另一项重叠，请先移除冲突项")
    ).toBeVisible();
    fireEvent.click(screen.getByLabelText("选择第 4 个词 light"));
    expect(screen.queryByText("查看草稿")).toBeNull();
    expect(screen.getByLabelText(/编辑待关联词义/u)).toBeVisible();
    open.mockRestore();
  }, 15_000);

  it("无分层数组的 A1 旧译文回填为高阶", () => {
    const sentence = existingSentence();
    sentence.level = "A1";
    delete (sentence as Partial<WordSentenceWritableV3>).zh_translations;
    render(
    ...
    expect(screen.getByLabelText("高阶中文译文")).toHaveValue("找到中心。");
  });
});
