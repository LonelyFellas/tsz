...
export const coverageCollectionConfig = {
  provider: "v8" as const,
  reportsDirectory: "./coverage",
  // 覆盖率聚焦运行时逻辑：各 package + web/admin 的业务与请求层。
  include: [
    "packages/shared/src/**",
    "packages/ui/src/**",
  ...
  exclude: [
    ...
    //    新版 word-creation 向导包含大量业务分支且已有组件测试，必须纳入统计。
...
export const fullCoverageConfig: CoverageV8Options = {
  ...
  reporter: ["text", "html"],
  // 包内逻辑维持 100%；应用层保留完整统计报告，但不设固定百分比门槛。
  thresholds: {
    "packages/**": {
      statements: 100,
      branches: 100,
...
export const shardCoverageConfig: CoverageV8Options = {
  ...coverageCollectionConfig,
  // 分片只收集数据；完整阈值和最终 reporter 在 merge 阶段统一执行。
  reporter: []
};
