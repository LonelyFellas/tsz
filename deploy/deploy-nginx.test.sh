...
remove_nginx_stage || fail "N02 trap cleanup after a failed install should still succeed"
...
# N05 暂存句柄不是换装函数建出来的路径时，兜底清理拒绝 rm -rf。
reset_case
mkdir -p "$test_dir/victim"
DEPLOY_NGINX_STAGE="$test_dir/victim"
if remove_nginx_stage 2>/dev/null; then
  fail "N05 unexpected stage path should be refused"
fi
