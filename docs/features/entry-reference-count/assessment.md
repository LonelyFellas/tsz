...
| 6   | **V3 短语成分**        | `v3_phrase_variant_component_usages.target_entry_id` | **短语把本单词作为成分**（短语 ↔ 单词的引用）            | ⚠️ 见下        | RESTRICT  |
...
### 缺口 A：删除被第 5 类引用的词条会返回 500（真实缺陷，已修）

> **2026-08-31 修正**：本节最初判断「第 5、6 类都会导致 500」，实查 shape check 后
> 第 6 类不成立——`lexicon_v3_phrase_components_shape_check` 要求
> `state='resolved'` 时 `target_publication_id IS NOT NULL`，即被短语引用的词条
> **必然已发布**；而已发布词条在删除时先被 `current_publication_id.is_some()`
> 拦下（409），走不到撞外键那步。第 6 类补进检查仅作防御，不是修复活跃缺陷。
>
> 第 5 类则确实可达：`sentence_associations` 的 `target_shape_check` 允许
> `state='linked'` 且不带 `target_publication_id`，即例句关联可指向未发布词条。
> 已三层验证：原始 500 → 仅补映射得 409 `reference_conflict` → 补检查后得
> 语义准确的 409 `entry_not_deletable`。修复见 tsz-rust `721bfee`。
...
| 是否按类型拆分                 | **第一版不拆**       | 管理员看到「被引用 3 次」的下一个问题是「谁」而非「哪种类型」——该信息由下钻明细回答，拆类型的边际价值被覆盖 |
...
1. **口径**：引用方词条去重 + 含草稿引用 + **第一版不拆类型**（只给 total）
2. **展示位置**：**只做列表**；词条详情页暂不展示
3. **缺口 A**：已单独修复并先行落地（tsz-rust `721bfee`）
4. **下钻明细**：**要做，但第一版只给前 5 条 preview**，不做完整分页

施工顺序见 [../admin-trash-cleanup/master-plan.md](../admin-trash-cleanup/master-plan.md)。
