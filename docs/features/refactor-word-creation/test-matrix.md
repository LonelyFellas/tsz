...
| --- | ---------------- | ---------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------ |
| T01 | contract         | V2 endpoint method/path/body | detect/create/impact/save/validate/publish/dialect suggestion                           | 相对 `/admin/lexicon` 路径、snake_case body、幂等 header 与权威 OpenAPI 一致                                 | P0     |
| T02 | unit/mock        | `center` 检测成功            | AmE 输入，词典匹配 `centre / center`，查重 clear                                        | 返回完整 suggestions、source_dialect=us；检测本身不创建资源                                                  | P0     |
| T03 | unit/mock        | 检测与创建矩阵               | duplicate、matched/not_found phrase、not_found word、builtin/smart unavailable、expired | 各状态可区分；matched 与 not_found phrase 可创建，重复/不可用/not_found word 阻断；错误与契约一致            | P0     |
...
| T07 | integration/UI   | 第 1 步检测与建草稿          | 点击检测、修改输入产生 stale response、重复、matched/not_found 短语、失败               | 只在点击时请求；旧响应不显示；短语进入 V2，阻断态不可继续；错误保留输入                                      | P0     |
...
| T36 | manual         | 视觉与长列表              | 1200/1440/1920px、50 条分页、长中英文名、页内表格滚动               | Step 2–4 均为顶部两行摘要且无左侧摘要栏；antd 布局无截断/横向溢出                        | P1     |
...
| T45 | regression     | 单词/短语入口不受配置 Tab 影响 | 点击创建单词/创建短语                       | 两者均进入 V2 向导并保留各自 kind；不以配置 Tab 改写 kind              | P0     |
...
| T55 | integration/UI | 顶部完成情况统计         | 无 word、空旧草稿、一个或多个语义区间        | Step 1 不显示摘要；Step 2–4 第二行显示“语义区间 1/N”，位置在词形变化与语法结构之间 | P0     |
...
| T61 | integration/UI | 完整目标零请求与缺失目标分批   | 目标已完整；超过 100 个英文释义和例句缺失；存在语法结构           | 完整目标纯切换零请求；缺失目标按 100 项顺序分批，且每批仅写请求快照内的 definition/example，不包含 grammar/ready/中文 | P0     |
...
- [ ] 使用多个基本词性和超过 100 个缺失项切换方言，确认选择跨 Tab 共用且按 100 项顺序分批。
...
| P1     | Phase 2 真实模式启用 archive、batch archive、phrase detection/create 与 evidence-only dialect suggestion；继续阻断 legacy create/saveContent/publish/delete 的列表动作与直达编辑路由；mock 与真实 capability 保持同形 | `apps/admin/src/features/dictionary/dataSource.test.ts`、`apps/admin/src/features/dictionary/SmartDictionary.test.tsx`、`apps/admin/src/features/dictionary/SmartDictionary.routing.test.tsx`、`apps/admin/src/pages/pages.test.tsx`                                                                                                       |
