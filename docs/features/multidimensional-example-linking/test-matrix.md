...
| #       | 层       | 场景                    | 输入/前置                                                     | 预期                                                                                  | 优先级 | 状态                                                           |
| ------- | -------- | ----------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------ | -------------------------------------------------------------- |
| FS-F-01 | 集成     | 打开生产抽屉            | 服务端 linked/pending/unresolved 投影                         | 不加载 Mock fixture；完整还原服务端状态；缺 exact identity 的兼容 linked 不得被过滤   | P0     | PASS：legacy linked 行数守恒回归                               |
...
| FS-F-06 | 集成     | 新例句第一步失败        | meanings save 4xx/5xx                                         | 不发 association 请求；保留完整抽屉草稿                                               | P0     | PASS：wizard 父级/抽屉状态回归                                 |
| FS-F-07 | 集成     | 新例句第二步失败        | sentence 已保存，association 4xx/5xx                          | 保持抽屉并提示 unresolved 可恢复状态；不伪装回滚                                      | P0     | PASS：drawer failure retention                                 |
| FS-F-08 | 集成     | revision/生命周期冲突   | replace/claim 返回 409                                        | 刷新真实 entry，旧本地 base revision 不重放                                           | P0     | PARTIAL：整组失败父级不变已回归；409 继续由既有 T5A 冲突流覆盖 |
| FS-F-09 | 集成     | Pending 创建目标跳转    | 点击“创建短语”                                                | `/words/create` 预填 headword；创建后保留 gloss/return_to 到目标 V3，并可返回来源     | P0     | PASS：UnifiedCreate/WordCreate/page 状态闭环                   |
| FS-F-10 | 集成     | 取消目标创建            | 从创建页返回未完成                                            | 原 Pending 仍在；不产生已绑定假状态                                                   | P0     | PASS：navigation-only boundary                                 |
| FS-F-11 | 集成     | Pending list 与 claim   | 分页、选择具体 sense、word/phrase 多 variant、claim 成功/冲突 | 唯一 variant 自动选；多值显式选；提交 current publication + variant；冲突以服务端为准 | P0     | PASS：panel exact identity/失败刷新回归                        |
...
| FS-F-14 | 可访问性 | 键盘完成范围与 Pending  | Tab/Enter、首尾词、gloss 编辑、保存/取消                      | 焦点顺序可见，控件有稳定名称，错误可定位                                              | P0     | PASS：token/表单/错误反馈组件回归                              |
...
| AD-C-06 | 契约       | 完整性与分页             | complete / overloaded、多候选/多sense                 | true total/cursor 与过载严格区分，cursor 绑定 generation                 | P0     | PASS：opaque cursor 契约/组件测试                                                                                                                    |
...
| AD-B-36 | HTTP/服务  | 成分用词完整发布快照     | 唯一词义、无匹配、多词义、历史激活                    | 已解析保存 identity/词形/释义；未解析保留词面；历史逐版恢复              | P0     | PARTIAL：exact association 的 publication/variant/component snapshot 持久化、回读和重存已覆盖；历史逐版 activation 尚无独立用例                      |
...
| AD-F-12 | 组件       | sense摘要与分页          | 候选/词义多页、generation变化、旧cursor               | 各位置独立加载/去重；旧cursor不混入                                      | P0     | PARTIAL：automatic `next_cursor` → 同片段 selected opaque cursor、load-more 合并与契约已覆盖；多位置多页去重及真实 generation 过期仍待 HTTP/并发验证 |
| AD-F-13 | 组件       | 手动无命中/草稿          | 多段无候选、草稿多词义/未完成                         | segments保留可转Pending；草稿UUID不进入正式payload                       | P0     | PASS：component                                                                                                                                      |
| AD-F-14 | 组件       | 完整请求失效防线         | A慢于B、方言/模式/segments变化、关闭抽屉              | 主请求与加载更多均纳入 abort+requestId+fingerprint；迟到/错 range 不合并 | P0     | PASS：abort/竞态/错 fingerprint/空页回归                                                                                                             |
...
| AD-R-06 | 回归       | 相同拼写不同方言成分     | BrE/AmE phrase surface 相同、component 不同           | 例句按 matched phrase variant 展示正确一侧，不合并两侧                   | P0     | PARTIAL：同拼写 BrE/AmE 成分独立编辑、显式 variant 选择与 exact identity 提交已覆盖；完整例句回读/历史展示的双方言端到端仍待验证                     |
