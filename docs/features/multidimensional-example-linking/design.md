...
- range 翻页 cursor 绑定 normalized surface、scope、publication generation 和排序键；词库变化后旧 cursor 返回稳定冲突或重新查询，不混页。
...
- 候选响应定义严格 sense 摘要：`sense_id、pos/base identity、level、gloss、publication_id`；候选分页与 sense 分页分别绑定 source fingerprint、generation 和稳定排序键。
...
- 例句 association 增加 `target_phrase_variant_id`，只引用实际命中的 common/BrE/AmE 短语词形；读取时从该 immutable publication variant 展开成分用词。这样同一短语在不同例句中不重复保存组件，也能随历史 publication 精确恢复。
...
- 例句 association 的 `target_publication_id + target_phrase_variant_id` 仍是下一切片；当前 discovery 已能精确返回 variant，但 association 表尚未固化该二元身份，因此不能把“刷新后永远恢复同一侧成分配置”描述成完成。
