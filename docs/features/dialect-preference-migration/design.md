...
> 产品口径一律以需求文档为准。
>
> **实施进度（2026-08-20）**：评估已于 2026-08-19 评审通过（三层模型 + 五条结论 + PR #134 作废）。
> **阶段 1–7 全部落地**：阶段 1 偏好内核与设置入口、阶段 2–5 四步 UI 收敛、
> 阶段 3 的语法结构直接单份（后端 P1 放宽后不再需要镜像 shim，原「阶段 6」并入）、
> 阶段 7 偏好持久化到服务端。落地过程中对本文的修正已就地改写并标注理由。
>
> 原先随本文提交的 `backend-proposal.md`（P1 / P2 的可转交提案）**已删除**：
> 提案已由 tsz-rust PR #35 实现，留着会误导后来者以为还在等后端。
> 提案原文仍完整保留在本文下方「提案 P1 / P2 / P3」三节里，可追溯。
>
> **⚠️ 唯一未闭合的交接事项：通知后端收敛 AI 补全（P1-b）。**
> 后端把 `content_completion` 对 `distinguish` 词条产双份同值镜像这件事**有意押后**，
> 等的就是前端阶段 3 发布。阶段 3 已随本 PR 落地，**请转达后端可以改成单份 `common` 了**——
> 否则收敛完成后 AI 补全仍会持续产出冗余的双条镜像数据。详见下方「后端答复」§P1-b。
>
> **核对基线**
>
> | 对象       | 版本 / 时间                                                                 |
...
**阶段 2–5 的收敛对 `@tsz/types` 零变更**：全部在前端行为层完成（读兼容 / 写收敛），
`EnglishTextV2` / `GrammarStructureV2` 的判别联合一个字没改——后端仍会返回 `distinguish`
（存量数据、AI 内容补全），类型里删掉那个分支等于让前端无法表达后端真实返回。

**阶段 7 有契约变更**（后端 P2 落地后，见下方「后端答复」）：

| 变更                                                                                     | 落点                                    |
| ---------------------------------------------------------------------------------------- | --------------------------------------- |
| `AdminDialectPreference` / `AdminPreferences` wire 类型；`AdminProfile.preferences` 必填 | `packages/types/src/admin.ts`           |
| `UpdateAdminPreferencesInput` / `UpdateAdminPreferencesResponse`                         | 同上                                    |
| `updateProfilePreferences` → `PATCH /admin/profile/preferences`                          | `packages/api-client/src/admin.ts`      |
| method / path / body 契约测试                                                            | `packages/api-client/src/admin.test.ts` |
| `openapi.snapshot.json`（45 → 46 条路径）                                                | 已随 PR #143 同步                       |

新端点在 spec 里，因此**不进 `endpoints.contract.test.ts` 的 PENDING 白名单**，
直接纳入正式校验。

以下三条提案是当初写给后端的原文，**均已由 tsz-rust PR #35 实现**，保留以便追溯。
（本 PR 不修改 tsz-rust 仓库任何内容。）
...
### 提案 P1（已由 tsz-rust #35 实现）· 放宽语法结构的方言形状校验
...
### 提案 P2（已由 tsz-rust #35 实现）· 管理员个人偏好持久化
...
### 提案 P3（已由 tsz-rust #35 实现）· 列表 headword 结构化
...
**语法结构同样参与丢弃**（后端 P1 放宽之后）：`collapseGrammarStructure` 把存量双条
折成单条 `common`，非偏好侧那条随节点删除。因此确认框把"语法结构 N 条"和英文释义、
例句并列写进将丢弃的清单里——只有非偏好侧确实写过、且与偏好侧不同的才计数，
空的或本来就是同值镜像的不打扰管理员。

**收敛必须给 `common` 变体换新节点 ID**：后端把方言编进了节点角色
（`text_variant_role(field_role, "en", dialect)`），沿用旧 uk 变体的 ID 只改 `dialect`
会被判 `node_binding_changed`，整个 meanings 保存 422。英文内容与语法结构同一条规矩。
...
| 阶段 7（后端偏好）     | revert 该 PR，前端回落到「本地存储即事实源」                                                                        | 无。偏好不写业务数据；后端那一列留着也无害                                                                      |
...
| ~~**6**~~ | ~~语法结构单份 wire~~               | **已并入阶段 3**：P1 先于前端落地，阶段 3 直接写单条 `common`，没有过 shim 也就无从删起                                                                             | —                               | —                                                                        |
| **7** ✅  | 偏好后端持久化                      | `@tsz/types` 加 wire、api-client 封装 `PATCH /admin/profile/preferences`、事实源切服务端，localStorage 降为离线缓存                                                 | 后端 P2                         | 契约测试与 snapshot 同步更新                                             |
...
阶段 7 已新增 profile preferences 的 method/path/body 契约测试
（`admin.test.ts` 的 `updateProfilePreferences → PATCH /profile/preferences`）。
...
| ~~语法结构镜像 shim 长期滞留~~ **已消解**                                                   | —                                  | 后端 P1 先于阶段 3 落地，前端从未把 shim 发出去；代码里的 `TODO(阶段 6)` 标记已随之删除                                      |
...
| ~~偏好只在本地，多设备不一致~~ **已消解**                                                   | —                                  | 阶段 7 已把事实源切到服务端（P2），本地存储降级为离线缓存                                                                    |
...
| #     | 问题                                                                                                            | 落点                  |
| ----- | --------------------------------------------------------------------------------------------------------------- | --------------------- |
| ~~1~~ | ~~偏好持久化提案 P2~~ **已实现**（tsz-rust #35）→ 阶段 7 已落地                                                 | 关闭                  |
| ~~2~~ | ~~语法结构校验放宽提案 P1~~ **已实现**（tsz-rust #35）→ 阶段 3 直接单份                                         | 关闭                  |
| 3     | 非偏好侧音标是否放宽为选填                                                                                      | 产品 + 后端；本轮不做 |
| 4     | 第 1 步是否开放"修正对侧拼写"                                                                                   | 产品；本轮不做        |
| ~~5~~ | ~~列表 headword 结构化提案 P3~~ **已实现**（`headword_variants`，tsz-rust #35 / §12）；前端按偏好重排列表尚未做 | 前端待排期            |
