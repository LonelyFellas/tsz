...
> **阶段 1–5 已落地**（偏好内核与设置入口 / 第 3 步释义例句收敛 /
> 语法结构收敛 + 镜像 shim / 第 1 步去决策 / 第 2 步偏好侧主导），
> 落地过程中对本文的修正已就地改写并标注理由；只剩阶段 6、7 两个后端前置项。
>
> **核对基线**
>
> | 对象       | 版本 / 时间                                                                 |
...
## 阶段 3 落地记录（2026-08-20）

1. **语法结构的镜像是保存时的事，编辑期只写偏好侧那一条**（与英文内容同一套节奏）。
   `writeGrammarText` 写单条、`mirrorGrammarStructure` 在 `toMeaningsWireContent`
   里补齐成后端要求的方言集合，节点 ID 逐方言复用，不每次保存换新节点。
2. **「会被覆盖的语法结构」必须按载入时的服务端副本统计**，不能按正在编辑的草稿。
   按草稿统计的话，编辑偏好侧后另一侧还留着上一次的镜像值，两者一比就被判成
   「即将覆盖美式内容」——每敲一个字弹一次确认框。实现里这是踩过一次的坑。
3. **确认框把语法结构一并计入**，初稿写的「语法结构不受影响」是错的（已在
   `requirements.md` 就地更正）：镜像取偏好侧内容，非偏好侧写过的不同文本会被覆盖。
   只在两侧文本**确实不同**时计入，空的或本来就是镜像的不打扰管理员。
4. **`grammarStructureIssueTarget` 收敛为恒返回 `content`**：编辑器只有一个输入框，
   `content.uk` / `content.us` 已经没有可定位的目标。
5. **少一条方言变体不再算未完成**——保存时镜像会补齐；只有正文为空才算。
   readiness 与校验都在归一化后的内容上跑。
...
## 阶段 5 落地记录（2026-08-20）

1. **只有「拼写区分」布局才折叠**。第 2 步有两套渲染：拼写区分时是双栏矩阵（可折叠一栏），
   拼写统一 + 音标区分时是共享拼写一格、两侧音标都在格内——那里没有可折叠的「列」，
   折叠只会让表头与格内内容对不上。后者只按偏好排序，不折叠。
2. **折叠侧必须能被校验定位自动展开**（code-review 抓到）。后端仍强制两侧齐全才能发布，
   所以问题完全可能落在折叠侧；不自动展开的话，点左栏「待完善项」页面毫无反应——
   目标节点不在 DOM 里，聚焦逻辑重试几次就静默放弃。现按「定位目标是否属于另一侧
   方言的变体或其读音」判断并自动展开。
3. **C4 的指名落在校验层而不是文案层**：`FormIssueTarget` 增加 `dialect` 字段，
   `baseFormIssueMessage` / 新增的 `derivedFormIssueMessage` 据此拼出
   「基准原形 · 美式缺少实际发音」「复数 · 英式缺少字典音标」。统一词形不出现方言字样。
   同侧统计避免把英式缺的和美式缺的混成一句。
4. **发音人偏好只作用于 `common`**：明确标了方言的内容配不到对应发音人时**不降级**
   （否则英式词形会用美式音朗读），保持禁用并说明；统一内容本来没有方言，
   偏好侧发音人缺席时回退目录默认，保证可试听。这与改造前对 uk/us 的行为一致。
5. **偏好读取放在 Provider 而不是每个控件**：一开始让每个 `PronunciationPreviewControls`
   各自 `useSyncExternalStore`，一页几百个实例把 4 个用例拖到超时。改为在
   `PronunciationPreviewProvider` 取一次、经 context 下发。
...
**语法结构同样计入丢弃**（阶段 3 修正）：它在阶段 6 前仍写两条同值镜像，
镜像的内容取偏好侧——所以非偏好侧写过的**不同**文本会被覆盖，这也是丢数据。
只在两侧文本确实不同时计入确认框；空的、或本来就是镜像的不打扰管理员。
计数取的是**载入时的服务端副本**而不是正在编辑的草稿：编辑只写偏好侧那一条，
拿草稿去比会把「你自己刚改的字」误判成「即将被覆盖的美式内容」，每改一个字弹一次框。
...
| 阶段 6（语法单份）  | revert；镜像 shim 回来       | 若后端已放宽，单条 `common` 的语法结构在旧前端里会被判为形状不符 → 需同步回滚顺序：**先回滚前端，再回滚后端放宽** |
...
| **3** ✅ | 第 3 步语法结构收敛（含镜像 shim） | 语法结构 UI 单份；`distinguish` 词条写双条同值镜像；校验同口径                                        | 阶段 2（同文件）                | 镜像满足后端现有校验                            |
