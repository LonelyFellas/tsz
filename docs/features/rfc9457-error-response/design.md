...
`current_revision`、`current_lifecycle_revision`、`usage_count`、`word_id`、
`part_of_speech_id`、`code`、`max_reachable_step`、`affected_node_ids` 和
`reference_locations`；每个成员自身也可选。词库字段级问题还可携带已校验的
`reference_location`。原
...
| N13 | 单元 | 合法 meta         | revision/lifecycle/reference 上下文 | problem.meta 与 HttpError.meta 同形 | P0     |
| N14 | 单元 | 畸形 meta         | 负数计数、空 ID、错误引用位置       | 丢弃 meta，不影响标准 Problem 解析  | P0     |
